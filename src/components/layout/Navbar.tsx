    ...
    <>
      <nav className={`sq-nav ${scrolled ? 'scrolled' : ''}`} style={style}>
        <style>{`
          ...
          /* Hide standard links on mobile and show bottom nav */
          .sq-mobile-nav {
            display: none;
          }
        ...
        {/* Desktop Middle Links - Portal-Specific */}
        <div className="sq-nav-links">
          ...
          {isStaff && (
            <>
              <a href="#assigned" onClick={(e) => handleTabClick(e, 'assigned')} className={`sq-nav-link ${activeTab === 'assigned' ? 'active' : ''}`}>Assigned Queues</a>
              <a href="#controls" onClick={(e) => handleTabClick(e, 'controls')} className={`sq-nav-link ${activeTab === 'controls' ? 'active' : ''}`}>Queue Controls</a>
            </>
          )}

          {isAdmin && (
            <>
              <a href="#queues" onClick={(e) => handleTabClick(e, 'overview')} className={`sq-nav-link ${activeTab === 'overview' ? 'active' : ''}`}>Queue Management</a>
              <a href="#staff" onClick={(e) => handleTabClick(e, 'manage')} className={`sq-nav-link ${activeTab === 'manage' ? 'active' : ''}`}>Staff Management</a>
              <a href="#analytics" onClick={(e) => handleTabClick(e, 'analytics')} className={`sq-nav-link ${activeTab === 'analytics' ? 'active' : ''}`}>Analytics</a>
            </>
          )}
        </div>
      ...
      {/* Mobile Bottom Tab Bar (Only visible on max-width: 768px) */}
      <div className="sq-mobile-nav">
        {isStudent && (
          <>
            <Link href="/dashboard" className={`sq-mobile-nav-link ${portal === 'student' ? 'active' : ''}`}>
              <LayoutDashboard size={20} />
              <span>Dashboard</span>
            </Link>
            <Link href="/token" className={`sq-mobile-nav-link ${portal === 'token' ? 'active' : ''}`}>
              <Ticket size={20} />
              <span>Token</span>
            </Link>
          </>
        ...
        {isStaff && (
          <>
            <a href="#assigned" onClick={(e) => handleTabClick(e, 'assigned')} className={`sq-mobile-nav-link ${activeTab === 'assigned' ? 'active' : ''}`}>
              <LayoutDashboard size={20} />
              <span>Assigned</span>
            </a>
            <a href="#controls" onClick={(e) => handleTabClick(e, 'controls')} className={`sq-mobile-nav-link ${activeTab === 'controls' ? 'active' : ''}`}>
              <Settings size={20} />
              <span>Controls</span>
            </a>
          </>
        ...
        {isAdmin && (
          <>
            <a href="#queues" onClick={(e) => handleTabClick(e, 'overview')} className={`sq-mobile-nav-link ${activeTab === 'overview' ? 'active' : ''}`}>
              <LayoutDashboard size={20} />
              <span>Overview</span>
            </a>
            <a href="#staff" onClick={(e) => handleTabClick(e, 'manage')} className={`sq-mobile-nav-link ${activeTab === 'manage' ? 'active' : ''}`}>
              <Users size={20} />
              <span>Staff</span>
            </a>
            <a href="#analytics" onClick={(e) => handleTabClick(e, 'analytics')} className={`sq-mobile-nav-link ${activeTab === 'analytics' ? 'active' : ''}`}>
              <BarChart3 size={20} />
              <span>Data</span>
            </a>
          </>
